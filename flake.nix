      ...
      in
      {
        ...
        packages.wasm = pkgs.stdenv.mkDerivation {
          ...
          buildPhase = ''
            ...
            # Manually compose emsdk
            cp -r $emsdk/* $out/emsdk
            cp -r $node/* $out/emsdk/node/18.20.3_64bit
            cp -r $wasm_binaries/* $out/emsdk/upstream

            # Manually add files generated by ./emsdk install 
            # Without these emsdk trying to fetch files again
            ...
            # Prepare cache directory for zig fetched depndencies
            mkdir -p $out/.cache/zig/p
            mkdir -p $out/.cache/zig/tmp

            # Fetch zig deps for itself
            # We use linkFarm to prefetch them all at once
